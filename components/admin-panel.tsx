...
import { Check, ExternalLink, Eye, FileCheck2, Flame, LoaderCircle, LogOut, ShieldCheck, Trash2, X } from "lucide-react";
...
import { formatDate, normalizeSubmissionMedia, safeMediaUrl } from "@/lib/utils";
    ...
    <section className="container-shell py-12 md:py-16">
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-w-2xl">
          ...
          {selected && <div className="mt-6"><div className="flex flex-wrap gap-2 text-xs font-bold"><span className="rounded-full bg-orange-100 px-3 py-1 text-orange-800">{typeLabels[selected.type]}</span><span className="rounded-full bg-stone-100 px-3 py-1 text-stone-600">{selected.authorName}</span>{selected.targetId && <span className="rounded-full bg-stone-100 px-3 py-1 text-stone-600">{selected.type === "CALL" ? "FC/Fansite: " : "Gửi "}{selected.targetId}</span>}</div>{selected.title && <h2 className="mt-5 text-xl font-black">{selected.title}</h2>}<p className="mt-3 whitespace-pre-line text-sm leading-7 text-stone-700">{selected.content}</p><SubmissionMediaGallery items={normalizeSubmissionMedia(selected.mediaItems, selected.mediaUrl, selected.mediaType)} alt="Media được gửi" contain className="mt-5 rounded-2xl" />{safeMediaUrl(selected.proofUrl) && <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-start gap-3"><FileCheck2 className="mt-0.5 shrink-0 text-amber-700" size={20} /><div className="min-w-0 flex-1"><strong className="block text-sm text-stone-900">File minh chứng - chỉ dành cho admin</strong><span className="mt-1 block truncate text-xs text-stone-500">{selected.proofFileName || "Minh chứng đính kèm"}</span><a href={safeMediaUrl(selected.proofUrl)!} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-xs font-black text-orange-700 hover:text-orange-900">Mở file minh chứng <ExternalLink size={12} /></a></div></div></div>}{selected.status === "PENDING" && <div className="mt-6 grid grid-cols-2 gap-3"><Button onClick={() => moderate(selected, "APPROVE")}><Check size={17} /> Phê duyệt</Button><Button variant="danger" onClick={() => moderate(selected, "REJECT")}><X size={17} /> Từ chối</Button></div>}</div>}
        </DialogContent>
      </Dialog>
    </section>
  );
