...
function ProfileCard({ person, index, kind }: { person: GuideArtist; index: number; kind: "artist" | "crew" }) {
  const label = kind === "artist" ? `Anh Tài ${String(index + 1).padStart(2, "0")}` : person.role || "Ekip chương trình";
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className="group cursor-pointer overflow-hidden rounded-[26px] border border-orange-100 bg-white p-3 text-left shadow-[0_12px_40px_rgba(120,53,15,.06)] transition hover:-translate-y-1 hover:border-orange-300 hover:shadow-[0_18px_48px_rgba(120,53,15,.12)]">
          <div className="relative aspect-[4/4.5] overflow-hidden rounded-[20px] bg-orange-100">
            ...
            <img src={person.imageUrl} alt={`Ảnh đại diện ${person.name}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-stone-950/75 to-transparent px-4 pb-4 pt-12">
              <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${person.hasProfile ? "bg-orange-500 text-white" : "bg-white/90 text-stone-600"}`}>
                {person.hasProfile ? "Đã có nội dung" : "Chờ cập nhật"}
              </span>
            </div>
          </div>
          <div className="px-2 pb-2 pt-4">
            <span className="text-[10px] font-black uppercase tracking-wider text-orange-600">{label}</span>
            <h3 className="mt-1 text-xl font-black tracking-tight text-stone-950">{person.name}</h3>
            <p className="mt-1 text-sm text-stone-500">{person.role || "Hồ sơ đang chờ admin cập nhật"}</p>
          </div>
        </button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl p-0 sm:p-0">
        <div className="grid md:grid-cols-[240px_1fr]">
          ...
          <div className="p-6 sm:p-8">
            <span className="eyebrow"><BookOpenText size={13} /> {kind === "artist" ? "Sổ tay Anh Tài" : "Người đồng hành"}</span>
            <DialogTitle className="mt-4">{person.name}</DialogTitle>
            <DialogDescription>{person.role || "Thông tin đang chờ cập nhật"}</DialogDescription>
            {person.hasProfile ? (
              <p className="mt-7 whitespace-pre-line border-t border-orange-100 pt-6 text-[15px] leading-7 text-stone-700">{person.content}</p>
            ) : (
              <div className="mt-7 rounded-2xl border border-dashed border-orange-200 bg-orange-50/60 px-5 py-8 text-center">
                ...
                <p className="mt-3 font-bold text-stone-800">Hồ sơ này còn đang để trống</p>
                <p className="mt-1 text-sm leading-6 text-stone-500">Admin sẽ bổ sung thông tin trong thời gian tới.</p>
              </div>
            )}
          </div>
        </div>
...
export function FanGuideContent({ content }: { content: GuideContent }) {
  const artists = content.artists.filter((person) => person.category === "ARTIST");
  const crew = content.artists.filter((person) => person.category === "CREW");
    ...
    <>
      <section className="container-shell py-20">
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {artists.map((artist, index) => <ProfileCard key={artist.slug} person={artist} index={index} kind="artist" />)}
        </div>

        <div className="mt-20 flex items-end justify-between gap-4 border-t border-orange-100 pt-16">
          <div>
            <span className="eyebrow"><Sparkles size={13} /> Những người đồng hành</span>
            <h2 className="mt-4 text-3xl font-black tracking-tight md:text-4xl">Ekip dẫn dắt hành trình</h2>
          </div>
          <span className="hidden text-sm font-semibold text-stone-400 md:block">4 thành viên · admin cập nhật nội dung</span>
        </div>
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {crew.map((person, index) => <ProfileCard key={person.slug} person={person} index={index} kind="crew" />)}
        </div>
      </section>
